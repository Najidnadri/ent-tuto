...
// NamedHobbies returns the Hobbies named value or an error if the edge was not
// loaded in eager-loading with this name.
func (u *User) NamedHobbies(name string) ([]*Hobby, error) {
	if u.Edges.namedHobbies == nil {
		return nil, &NotLoadedError{edge: name}
	}
	nodes, ok := u.Edges.namedHobbies[name]
	if !ok {
		return nil, &NotLoadedError{edge: name}
	}
	return nodes, nil
...
func (u *User) appendNamedHobbies(name string, edges ...*Hobby) {
	if u.Edges.namedHobbies == nil {
		u.Edges.namedHobbies = make(map[string][]*Hobby)
	}
	if len(edges) == 0 {
		u.Edges.namedHobbies[name] = []*Hobby{}
	} else {
		u.Edges.namedHobbies[name] = append(u.Edges.namedHobbies[name], edges...)
	}
}
