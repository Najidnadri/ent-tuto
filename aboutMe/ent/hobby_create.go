...
func (hc *HobbyCreate) AddOwnerIDs(ids ...int) *HobbyCreate {
	hc.mutation.AddOwnerIDs(ids...)
	return hc
}

// AddOwners adds the "owners" edges to the User entity.
func (hc *HobbyCreate) AddOwners(u ...*User) *HobbyCreate {
	ids := make([]int, len(u))
	for i := range u {
		ids[i] = u[i].ID
	}
	return hc.AddOwnerIDs(ids...)
}
...
func (hc *HobbyCreate) createSpec() (*Hobby, *sqlgraph.CreateSpec) {
	...
	if nodes := hc.mutation.OwnersIDs(); len(nodes) > 0 {
