...
type UserQuery struct {
	...
	withNamedHobbies map[string]*HobbyQuery
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	for name, query := range uq.withNamedHobbies {
		if err := uq.loadHobbies(ctx, query, nodes,
			func(n *User) { n.appendNamedHobbies(name) },
			func(n *User, e *Hobby) { n.appendNamedHobbies(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadHobbies(ctx context.Context, query *HobbyQuery, nodes []*User, init func(*User), assign func(*User, *Hobby)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(user.HobbiesTable)
		s.Join(joinT).On(s.C(hobby.FieldID), joinT.C(user.HobbiesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(user.HobbiesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(user.HobbiesPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
...
// WithNamedHobbies tells the query-builder to eager-load the nodes that are connected to the "hobbies"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithNamedHobbies(name string, opts ...func(*HobbyQuery)) *UserQuery {
	query := (&HobbyClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if uq.withNamedHobbies == nil {
		uq.withNamedHobbies = make(map[string]*HobbyQuery)
	}
	uq.withNamedHobbies[name] = query
	return uq
}
